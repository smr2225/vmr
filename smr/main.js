...
async function loadPage(page) {
    ...
    try {
        ...
        // *** ДОБАВИТЬ: Загружаем и выполняем JS страницы ***
...
// *** НОВАЯ ФУНКЦИЯ: Загрузка JS страницы ***
async function loadPageJS(page) {
    try {
        // Удаляем старый скрипт если есть
        const oldScript = document.querySelector(`script[data-page-script="${page}"]`);
        if (oldScript) {
            oldScript.remove();
        }
        
        // Проверяем существование JS файла
        const response = await fetch(`${page}/${page}.js`);
        if (response.ok) {
            const jsCode = await response.text();
            
            // Создаём и выполняем скрипт
            const script = document.createElement('script');
            script.setAttribute('data-page-script', page);
            script.textContent = jsCode;
            document.body.appendChild(script);
            
            console.log(`JS для ${page} загружен и выполнен`);
            
            // Если это календарь - инициализируем его
            if (page === 'clnd' && typeof window.initClndCalendar === 'function') {
                console.log('Вызываем initClndCalendar()');
                window.initClndCalendar();
            }
        }
    } catch (error) {
        console.log(`JS для ${page} не найден или ошибка:`, error);
    }
}
...
// Если календарь - перерисовываем
if (currentPage === 'clnd' && typeof window.renderClndCalendars === 'function') {
    window.renderClndCalendars();
}
